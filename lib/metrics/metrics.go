...
var (
	...
	BytesForwarded = promauto.NewCounterVec(prometheus.CounterOpts{
		Name: "fazantix_source_bytes_forwarded_total",
		Help: "Total number of bytes forwarded in frames from a source",
	}, []string{"name"})
)
...
func NewSourceMetrics(name string) SourceMetrics {
	s := SourceMetrics{
		...
		BytesForwarded:  BytesForwarded.WithLabelValues(name),
	}
	s.FramesForwarded.Add(0)
	s.BytesForwarded.Add(0)
	return s
}
